...
from utils.components.summarizing_functions import (
    ModelTraining,
    SummaryMaker,
        ...
        dbc.Accordion(
            [
                dbc.AccordionItem(
                    ...
                    title="2. Select the section you want to summarize",
                ),
            ],
            start_collapsed=True,
            style={"margin": 30},
        ...
        dbc.Accordion(
            [
                dbc.AccordionItem(
                            ...
                            [
                                dbc.Row(
                                    [
                                        dbc.Col(
                                            [
                                                app_components.create_text_area(
                                                    component_id="input-summary-text",
                                                    placeholder="Review the section's content...",
                                                )
                                            ],
                                            xs=12,
                                            sm=12,
                    ...
                    title="3. Review the section's text",
                ),
            ],
            start_collapsed=True,
            style={"margin": 30},
...
@callback(
    ...
    [
        Input("upload-pdf-file", "contents"),
        Input("upload-pdf-file", "filename"),
    ],
    [State("sessh", "data")],
...
def read_meta_data(list_of_contents, filename, sessh_data):
    ...
    if trigger_id == "upload-pdf-file":
        if list_of_contents is not None:
            pdf_contents, sections = read_pdf_file(list_of_contents, filename)
...
@callback(
    [
        Output("sessh", "data"),
        Output("input-summary-text", "value"),
        Output("export-table", "data"),
        Output("export-table", "columns"),
    ],
    [
        Input("dpdn-section", "value"),
...
def summarize_data(section, n_clicks_gen, system_prompt, sessh_data):
    ...
    if trigger_id == "dpdn-section":
        ...
        if train_model is True:
            model = ModelTraining(
                paper=list_of_training_texts, summary=list_of_summary_texts
            )

        maker = SummaryMaker(
            paper=list_of_training_texts,
            summary=list_of_summary_texts,
            text=user_input,
        ...
        return (
            sessh_data,
            sessh_data["user_input"],
            export_df.to_dict(orient="records"),
            [{"id": x, "name": x} for x in column_lst],
        )
