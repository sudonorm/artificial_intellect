...
class SummaryMaker:
    def __init__(self, paper: list, summary: list, text, prompt_input):
        self.papers = paper
        self.summaries = summary
        # self.model_trainer = ModelTraining(paper, summary)
        # self.model, self.vectorizer = self.model_trainer.train_model()
        self.text = text
        self.prompt_input = prompt_input

        # # Check if model and vectorizer exist in disk
        # if os.path.exists("./utils/components/model.pkl") and os.path.exists(
        #     "./utils/components/vectorizer.pkl"
        # ):
    ...
    def generate_summary(self, llm_type: str = MODEL, n: int = 200):
        ...
        SYSTEM_PROMPT = self.prompt_input
